...
import { setFilterIndividualAll } from 'redux/actions/report'
...
export default function Filter_individual_all({modal_data,profile}) {
    ...
    useEffect(() => {
        setstate({
            rangeValue:filter_individual_all.rangeValue,
            textPeriode:filter_individual_all.textPeriode,
            periode:filter_individual_all.periode,
            tribes:filter_individual_all.tribes
        })
    }, [])
    const makeText = m => {
    ...
    const onClickApply=async ()=>{
        await dispatch(setFilterIndividualAll({
        ...
        await dispatch(setFilterIndividualAll({tribes:state.tribes}))
        let new_tribes=[]
        state.tribes.map((d)=>{
