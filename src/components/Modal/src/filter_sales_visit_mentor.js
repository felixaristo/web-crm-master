...
import { setFilterSalesVisitMentor } from 'redux/actions/account'
...
export default function Filter_sales_visit_mentor_mentor({modal_data,profile}) {
    ...
    let {filter_sales_visit_mentor}=account
    useEffect(() => {
        setstate({
            rangeValue:filter_sales_visit_mentor.rangeValue,
            textPeriode:filter_sales_visit_mentor.textPeriode,
            periode:filter_sales_visit_mentor.periode,
            tribes:filter_sales_visit_mentor.tribes
        })
    }, [])
    const makeText = m => {
    ...
    const onClickApply=async ()=>{
        await dispatch(setFilterSalesVisitMentor({
        ...
        await dispatch(setFilterSalesVisitMentor({tribes:state.tribes}))
        let new_tribes=[]
        state.tribes.map((d)=>{
