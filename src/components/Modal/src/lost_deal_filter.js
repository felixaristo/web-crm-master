...
import {setFilter,getLostDeal} from 'redux/actions/pipeline'
...
export default function Payment_period(props) {
    ...
    const onClickSave=async ()=>{
        ...
        let res=await dispatch(getLostDeal(props.token,`/${periode.fromMonth}/${periode.toMonth}/${tribe}/${segment}/${rm.value}/1/10/${client.search.length>0?client.search:'*'}`))
        if(res){
