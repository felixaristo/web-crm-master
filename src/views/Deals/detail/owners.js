...
export default function Owners(props) {
    console.log("Updated bug looping 2, 21-03-2022")
    ...
    const configProposalValueChange=async ()=>{
        ...
        if(tribes.length>0&&total_nominal_tribe!==proposal.proposalValue){
            dispatch(addOwnerWithoutAlert(props.token,data))
        }
        if(rms.length>0&&total_nominal_rm!==proposal.proposalValue){
            dispatch(addOwnerWithoutAlert(props.token,data))
        }
    }
    const detailOwner=()=>{
